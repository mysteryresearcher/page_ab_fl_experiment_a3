...
class CompressorType:
    ...
    PERMK_COMPRESSOR = 9         # Permute-K compressor (D >= N)
#======================================================================================================================
class Compressor:
    ...
    def fullName(self):
        ...
        if self.compressorType == CompressorType.RANDK_COMPRESSOR: return f"RandK [K={self.K},D={self.D}]"
        ...
        if self.compressorType == CompressorType.TOPK_COMPRESSOR: return f"TopK [K={self.K},D={self.D}]"
        if self.compressorType == CompressorType.RANK_K_COMPRESSOR: return f"Rank [K={self.K},D={self.D}]"
        if self.compressorType == CompressorType.PERMK_COMPRESSOR: return f"PermK [D={self.D}]"
    ...
    def makePermKCompressor(self, D):
        """Make Permute-K sparsification with forcing selecting u.a.r. K non-zeros component from all D components"""
        # E[|C(x)|^2]=(d*d)/(k*k) * E[sum(zi*ei*xi)^2)] = (d*d)/(k*k) * k/d *|x|^2 = d/k * (x^2) = (w + 1) (x^2) => w = d/k-1
        self.compressorType = CompressorType.PERMK_COMPRESSOR
        self.D = D
        self.w = 0
        self.resetStats()
    ...
    def generateCompressPattern(self, rndgen, device, clientId, H):
        ...
        if self.compressorType == CompressorType.IDENTICAL:
            ...
            pass
        if self.compressorType == CompressorType.PERMK_COMPRESSOR:
            if clientId != -1:
                # Initialize server information relative last permutation
                if 'client_permute_is_for_round' not in H:
                    H['client_permute_is_for_round'] = H['current_round'] - 1

                # Potential update of permutate compressor
                if H['client_permute_is_for_round'] != H['current_round']:
                    H['client_permute_is_for_round'] = H['current_round']
                    H['coordinates_permute'] = rndgen.permutation(self.D)
                    H['coordinates_per_client'] = {}

                    nClients = len(H["sampled_clients_in_round"])
                    componensPerClient = self.D // nClients

                    # Compressor for D >= N
                    assert H["D"] >= nClients

                    # Spread out indicies for clients
                    for c in range(nClients):
                        cIndex = H["sampled_clients_in_round"][c]
                        coordinates = H['coordinates_permute'][(c) * componensPerClient : (c+1) * componensPerClient]
                        H['coordinates_per_client'][cIndex] = coordinates.tolist()

                    # Figure out what what to do with the rest
                    residual = H['coordinates_permute'][(self.D // nClients) * nClients:]
                    clientsPermutation = rndgen.permutation(nClients)

                    for i in range(len(residual)):
                        c = clientsPermutation[i]
                        cIndex = H["sampled_clients_in_round"][c]
                        H['coordinates_per_client'][cIndex].append(residual[i])

                self.S = H['coordinates_per_client'][clientId]
                self.nClients = len(H['coordinates_per_client'].keys())
    ...
    def compressVector(self, x):
        ...
        elif self.compressorType == CompressorType.PERMK_COMPRESSOR:
            S = self.S
            out = torch.zeros_like(x)
            out[S] = (self.nClients) * x[S]
            # We assume that we don't need to send indicies
            self.last_need_to_send_advance = len(S)
...
class ProbabilisticSwitchingCompressor:
    ...
    def compressVector(self, x, rndgen):
        ...
        return None
#======================================================================================================================
def initCompressorExplicit(compressorCmdLine, D, KExplicit):
    params = compressorCmdLine.split(":")
    c = Compressor()
    if params[0] == "ident":
        c.makeIdenticalCompressor()
    elif params[0] == "randk":
        c.makeRandKCompressor(D, KExplicit)
        ...
        c.makeTopKCompressor(D, KExplicit)
    elif params[0] == "permk":
        c.makePermKCompressor(D, KExplicit)
    elif params[0] == "permkv2":
        c.makePermKV2Compressor(D, KExplicit)
    else:
