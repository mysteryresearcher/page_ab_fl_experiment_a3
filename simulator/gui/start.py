...
def showArtificalDatasetParameters(show = True):
    ...
    if show:
        ...
        configView.lblHomogeneousDS_L1.setVisible(True)
        configView.lblHomogeneousDS_L2.setVisible(True)
        configView.spbHomogeneousDS_L1.setVisible(True)
        configView.spbHomogeneousDS_L2.setVisible(True)

        configView.lblSolutionPerturbation.setVisible(True)
        configView.edtSolPerturbation.setVisible(True)
        ...
        configView.lblHomogeneousDS_L1.setVisible(False)
        configView.lblHomogeneousDS_L2.setVisible(False)
        configView.spbHomogeneousDS_L1.setVisible(False)
        configView.spbHomogeneousDS_L2.setVisible(False)

        configView.lblSolutionPerturbation.setVisible(False)
        configView.edtSolPerturbation.setVisible(False)
...
def onCmdLineGenerationForCurrentExperiment(withNewLines):
    ...
    if True:
        ...
        outputIsPresented = False

        for item in cmds:
            item = item.strip()

            if item == "--out":
                outputIsPresented = True
        ...
        extra_args = ""
        if not outputIsPresented:
            extra_args = f'--out "{job_id}.bin"'
...
#            send_scalars_to_master.append(client_stats['send_scalars_to_master'])
    ...
    # For debugging
    for k in ['hashed_step_size', 'hashed_Vpm', 'hashed_Vp', 'hashed_Vm', 'hashed_V', 'compressor_fullname', 'hashed_Li',
              'hashed_L', 'hashed_max_Li', 'hashed_A_compressor', 'hashed_B_compressor', 'hashed_Li_all_clients_data_points', 'hashed_L_minus', 'hashed_L_plus_w', 'hashed_L_plus_minus_w', 'hashed_L_plus_minus_w_2', 'hashed_used_L_plus_minus_w']:
        if k in H:
            simulations_stats[job_id][k] = H[k]
...
def cbxClientSamlingTypeSelection(index):
    ...
    if algo == "uniform sampling" or algo == "uniform sampling with replacement" or algo == "important sampling with replacement":
...
def currentCommandLineFromGUI():
    ...
    if sampling_index_type == 0:
        ...
        cmdline.append("--client-sampling-poisson")
        cmdline.append(configView.edtPoissonSampling.text())
    elif sampling_index_type == 3:
        cmdline.append("--client-sampling-type")
        cmdline.append("important-sampling-with-replacement")
        cmdline.append("--num-clients-per-round")
        cmdline.append(configView.edtNumClientsPerRound.text())
    elif sampling_index_type == 4:
        cmdline.append("--client-sampling-type")
        cmdline.append("uniform-sampling-with-replacement")
        cmdline.append("--num-clients-per-round")
        cmdline.append(configView.edtNumClientsPerRound.text())
    ...
    if ds == "generated_for_quadratic_minimization":
        dsGenSpec = []
        dsGenSpec.append(f"homogeneous_level_1:{str(configView.spbHomogeneousDS_L1.value())}")
        dsGenSpec.append(f"homogeneous_level_2:{str(configView.spbHomogeneousDS_L2.value())}")
        dsGenSpec.append(f"b_perturb:{str(configView.edtSolPerturbation.text())}")
...
def onExecuteScript():
    for i in range(1,219+1):
        analysisView.tabWidget.setCurrentIndex(i-1)
        onTabWidgetInAnalysisChanged()
        onRemoveAllExperiments()
        onAddExperimentResultsFromFolder(f"./../{i}")
        sortSimulationsByStatsKey("group-name")

        analysisView.tblExperiments.selectAll()
        onPlotResults(export_legend_to_file = False)
        #onTimerEvent()
        pass
...
def onRemoveAllExperiments():
    ...
    if total_commands != 0:
        ...
        global  simulations_stats, simulations_result, simulations_by_rows
        # ==========================================================
        raise_question = False
        sim_stats_res_lock.acquire()
        for job_id, job_stats in simulations_stats.items():
            if "source" not in job_stats:
                raise_question = True
                break
        sim_stats_res_lock.release()
        # ==========================================================
        ...
        while analysisView.tblExperiments.rowCount() > 0:
            analysisView.tblExperiments.removeRow(0)
        # ==========================================================
...
def onAddExperimentResultsFromFolder(folderPath):
    import os

    global simulations_by_rows, simulations_stats, simulations_result

    fileNames = [os.path.join(folderPath, f) for f in os.listdir(folderPath) if os.path.isfile(os.path.join(folderPath, f))]
    ...
    for fileName in fileNames:
        ...
        for job_id in list_job_ids:
            ...
            if job_id in simulations_stats:
                ...
                #print(H["args"])
                # HACK
                #H["group-name"] += "-" + H["args"].dataset_generation_spec
                #H["args"].group_name += "-" + H["args"].dataset_generation_spec
...
def onLoadExperimentResults():
    ...
    if total_commands != 0:
        dlg = QMessageBox()
        dlg.setWindowTitle("About current experiments")
        dlg.setText(f"There are {total_commands} current execution. Please stop them first.")
        dlg.setStandardButtons(QMessageBox.Ok)
        dlg.setIcon(QMessageBox.Information)
        dlg.exec_()
...
def toAxisName(axis:str, destinationAxis:str):
    """Specialized translation of axis names for more nice names for axis names in a final plots"""

    # GUI configured in such way that we don't want ot show axis titles
    if analysisView.cbxAxisTitle.currentText() == "show: OX axis title" and destinationAxis == "OY":
        return  ""
    if analysisView.cbxAxisTitle.currentText() == "show: OY axis title" and destinationAxis == "OX":
        return ""
    if analysisView.cbxAxisTitle.currentText() == "show: No axis title":
        return ""
    ...
    if axis == "Norm of function gradient square(validation)":
       return "$||\\nabla f(x^k)||^2$ (validation)"

    if axis == "Norm of function gradient (validation)":
       return "${||\\nabla f(x^k)||}$ (validation)"
    ...
    if axis == "Li constant for datapoints":
        return "${L_{f_i}}$"

    if axis == "Li constant":
        return "$L_i$ smooth constant"

    if axis == "Norm of iterate at round start (train)":
        return "${||x||}$"

    if axis == "Norm square of iterate at round start (train)":
        return "${||x||}^2$"

    if axis == "Full gradient oracles (train)":
        return "Epochs"

    if axis == "Sample gradient oracles (train)":
        return r"# of gradient calculations"
...
def onPlotResults(export_legend_to_file):
    ...
    elif analysisView.cbxLineStyle.currentIndex() == 4:
        linestyle = ["dashdot"] * len(job_ids)

    # https://matplotlib.org/stable/gallery/color/named_colors.html
    color = ["tab:red", "tab:green", "tab:blue", "tab:orange", "tab:purple"]#, "tab:purple", "tab:brown", "tab:pink", "tab:gray", "tab:olive", "tab:cyan"]
    ...
    plots = []
    custom_markers_freq = [int(freq) for freq in analysisView.edtMarkersFrequency.text().split(",") if len(freq.strip())>0]
    ...
    for g, job_id in enumerate(job_ids):
        ...
        custom_per_round = [0.0] * rounds
        custom_acum = [0.0] * rounds
        ...
        for round in range(rounds):
            ...
            for client_id, client_summary in round_info['client_states'].items():
                ...
                # Custom compute counter
                if "error_in_gradient" in client_summary['client_state']:
                    custom_per_round[round] += client_summary['client_state']['error_in_gradient'].norm().item()

        for round in range(rounds):
            custom_acum[round] = custom_per_round[round]
        ...
        for i in range(1,rounds):
            ...
            custom_acum[i] += custom_acum[i-1]

        rounds_with_actual_info = 0
        stop_round_traversing = False

        # Ability to have axis not indexed by rounds
        plot_as_graphic = True
        plot_as_bar = False

        if xAxisType == 'Clients':
            X = range(H['total_clients'])
            plot_as_graphic = False
            plot_as_bar = True

        if yAxisType == 'Clients':
            Y = range(H['total_clients'])
            plot_as_graphic = False
            plot_as_bar = True

        if xAxisType == 'Datapoints':
            X = range(len(H['hashed_Li_all_clients_data_points']))
            Y = np.zeros((len(X))) # HACK
            plot_as_graphic = False
            plot_as_bar = True

        if yAxisType == 'Datapoints':
            Y = range(len(H['hashed_Li_all_clients_data_points']))
            plot_as_graphic = False
            plot_as_bar = True


        # Traverse across completed rounds and collect what we can
        for round in range(min(len(X), len(Y))):
            round_info = None

            if round in H['history']:
                round_info = H['history'][round]

            for axis_type, axis_values in ( (xAxisType, X), (yAxisType, Y) ):

                if axis_type == "Client Error Accum":
                    axis_values[round] = custom_acum[round]

                if axis_type == "Client Error Per Round":
                    axis_values[round] = custom_per_round[round]

                if axis_type == "Clients":
                    pass

                if axis_type == "Datapoints":
                    pass

                if axis_type == "Li constant":
                    cl = round
                    if 'hashed_Li' not in H:
                        stop_round_traversing = True
                        break
                    axis_values[cl] = H['hashed_Li'][cl]

                if axis_type == "Li constant for datapoints":
                    cl = round
                    if 'hashed_Li_all_clients_data_points' not in H:
                        stop_round_traversing = True
                        break
                    axis_values[cl] = H['hashed_Li_all_clients_data_points'][cl]
                ...
                if axis_type == "Norm square of iterate at round start (train)":
                    field = "x_before_round"
                    if field not in round_info:
                        stop_round_traversing = True
                        break
                    axis_values[round] = (round_info[field]) * (round_info[field])
        ...
        plot_args["num_clients_per_round"]  = H['args'].num_clients_per_round
        plot_args["client_sampling_type"]   = H['args'].client_sampling_type

        # =================================================================================================================
        plot_args["formatLabelString"] = analysisView.edtLegendFormatString.text()
        # =================================================================================================================
        ...
        for key, value in H.items():
            ...
            if type(value) == torch.Tensor and value.numel() == 1:
                plot_args[key] = value.item()

        if 'page_ab_synthetic' in H["args"].algorithm_options.split(","):
            plot_args['algorithm'] += "-AB"
        else:
            plot_args['comment'] += "Vanilla PAGE"

            plot_args["formatLabelString"] = plot_args["formatLabelString"].replace(", $L_{{\pm,w}}$={hashed_used_L_plus_minus_w:.2f}, $L_+$={hashed_L_plus_w:.2f}", "")
            plot_args["formatLabelString"] = plot_args["formatLabelString"].replace("$L_{{\pm,w}}$={hashed_used_L_plus_minus_w:.2f}, $L_+$={hashed_L_plus_w:.2f},", "")
            plot_args["formatLabelString"] = plot_args["formatLabelString"].replace(",$L_{{\pm,w}}$={hashed_used_L_plus_minus_w:.2f},$L_+$={hashed_L_plus_w:.2f}", "")
            plot_args["formatLabelString"] = plot_args["formatLabelString"].replace("$L_{{\pm,w}}$={hashed_used_L_plus_minus_w:.2f},$L_+$={hashed_L_plus_w:.2f},", "")

        if ("internal_sgd:sgd-multi" in H["args"].algorithm_options.split(",")) and ("sgd_multi_case_1" in H["args"].algorithm_options.split(",")) and ("page_ab_synthetic" in H["args"].algorithm_options.split(",")):
            plot_args["comment"] += "Uniform With Replacement"
        if ("internal_sgd:sgd-multi" in H["args"].algorithm_options.split(",")) and ("sgd_multi_case_2" in H["args"].algorithm_options.split(",")) and ("page_ab_synthetic" in H["args"].algorithm_options.split(",")):
            plot_args["comment"] += "Importance"
        if ("internal_sgd:sgd-nice" in H["args"].algorithm_options.split(",")) and ("page_ab_synthetic" in H["args"].algorithm_options.split(",")):
            plot_args["comment"] += "Nice"

        plot_args["comment_clients"] = ""
        if H["args"].client_sampling_type == "important-sampling-with-replacement":
            plot_args["comment_clients"] += "Importance"
        if H["args"].client_sampling_type == "uniform-sampling-with-replacement":
            plot_args["comment_clients"] += "Uniform With Replacement"
        if H["args"].client_sampling_type == "uniform":
            plot_args["comment_clients"] += "Nice"
        if H["args"].client_sampling_type == "poisson" or H["args"].client_sampling_type == "poisson-no-empty":
            plot_args["comment_clients"] += "Independent"

        if "hashed_used_L_plus_minus_w" not in plot_args:
            plot_args["hashed_used_L_plus_minus_w"] = 0.0
        if "hashed_L_plus_w" not in plot_args:
            plot_args["hashed_L_plus_w"] = 0.0
        if "hashed_L_minus" not in plot_args:
            plot_args["hashed_L_minus"] = 0.0
        ...
        num_clients = H['hashed_Li_all_clients_data_points'].shape[0]
        clients_data_points = H['hashed_Li_all_clients_data_points'].shape[1]
        plot_args["opt-tau-samples"] = math.ceil( (float(plot_args["opt-tau"].replace("%", "")) / 100.0) * clients_data_points )
        ...
        # =================================================================================================================
        markers_numbers = [m for m in [9, 10, 11, 12, 13, 15, 16, 17, 19]]
        ...
        if g < len(custom_markers_freq):
            mark_every = custom_markers_freq[g]

        plot_args["markevery"] = mark_every
        plot_args["marker"] = marker[g % len(marker)]
        plot_args["linestyle"] = linestyle[g % len(linestyle)]

        ax.set_xlabel(toAxisName(xAxisType, "OX"), fontdict = {'fontsize' : fontSize} )
        ax.set_ylabel(toAxisName(yAxisType, "OY"), fontdict = {'fontsize' : fontSize} )

        if analysisView.cbxShowTicksOX.checkState() != Qt.Checked:
            # Optionally remove ticks for OX axis (can be usefull for some plots) when there are just too much elements in OX.
            ax.set_xticks([])
    ...
    # Usual plots
    # ===================================================================================================================================
    rasterizedPlots = analysisView.spbColorShift.value()
    ...
    if analysisView.cbxLegendLogScalingY.checkState() == Qt.Checked and doNotAggregate:
        for p in plots:
            try:
                # You can mix substituion and curly braces in latex in plots titles via using ${{a}}_{{1}}$ notation
                formatedLabel = p["formatLabelString"].format(**p)
            except:
                showError("The legend contains unknown substitution variables")
                return

            if not analysisView.cbxAutoDefineAxisLimits.isChecked():
                try:
                    yMin = eval(analysisView.edtYmin.text())
                    yMax = eval(analysisView.edtYmax.text())
                    xMin = eval(analysisView.edtXmin.text())
                    xMax = eval(analysisView.edtXmax.text())
                    ax.set_ylim([min(yMin, yMax), max(yMax, yMax)])
                    ax.set_xlim([min(xMin, xMax), max(xMin, xMax)])
                except:
                    showError("Please specify limits as a floats literals parsable by Python")
                    return

            if plot_as_bar:
                ax.bar(p["X"], p["Y"], linestyle = p["linestyle"], label = formatedLabel, color = color[rasterizedPlots % len(color)])
                if analysisView.cbxShowTicksOX.checkState() == Qt.Checked:
                    ax.axes.xaxis.set_ticks(p["X"])

            elif plot_as_graphic:
    ...
    if analysisView.cbxLegendLogScalingY.checkState() != Qt.Checked and doNotAggregate:
        for p in plots:
            ...
            if plot_as_bar:
                ...
                ax.plot(p["X"], p["Y"], marker = p["marker"], markevery = p["markevery"], linestyle = p["linestyle"], label = formatedLabel, color = color[rasterizedPlots % len(color)])
    ...
    plot_best_and_worst = (analysisView.cbxDrawBestAndWorst.checkState() == Qt.Checked)       # Plot or not best and worst

    if not doNotAggregate:
        groups_min_y    = {}     # Estimation of min for Y quantity
        groups_max_y    = {}     # Estimation of min for Y quantity
        ...
        for p in plots:
            ...
            if group not in groups_and_styles:
                ...
                groups_min_y[group] = np.array(p["Y"])
                groups_max_y[group] = np.array(p["Y"])
                ...
                groups_min_y[group] = np.minimum(np.array(p["Y"][0:min_len]), groups_min_y[group][0:min_len])
                groups_max_y[group] = np.maximum(np.array(p["Y"][0:min_len]), groups_max_y[group][0:min_len])

                groups_n[group]    += 1

        # Reshuffling of groups can be placed here
        def swapElementsInVector(vec, i, j):
            tmp = vec[i]
            vec[i] = vec[j]
            vec[j] = tmp

        # Hack
        rasterizedPlotsIndicies = [i for i in range(len(groups_order))]
        #swapElementsInVector(groups_order, 2, 3)
        #swapElementsInVector(rasterizedPlotsIndicies, 2, 3)

        for group in groups_order:
            groups_sigmay = np.sqrt(np.abs((groups_mysqr[group] - groups_my[group]**2)))
            ...
            if plot_as_bar:
                ax.bar(x_filtered, groups_my[group], linestyle=p["linestyle"], label=formatedLabel, color=color[rasterizedPlotsIndicies[rasterizedPlots] % len(color)])
                if analysisView.cbxShowTicksOX.checkState() == Qt.Checked:
                    ax.axes.xaxis.set_ticks(p["X"])

            else:
                if analysisView.cbxLegendLogScalingY.checkState() == Qt.Checked:
                    ax.semilogy(x_filtered, groups_my[group], marker = p["marker"], markevery = p["markevery"], linestyle = p["linestyle"], label = formatedLabel, color = color[rasterizedPlotsIndicies[rasterizedPlots] % len(color)])

                if analysisView.cbxLegendLogScalingY.checkState() != Qt.Checked:
                    ax.plot(x_filtered, groups_my[group], marker = p["marker"], markevery = p["markevery"], linestyle = p["linestyle"], label = formatedLabel, color = color[rasterizedPlotsIndicies[rasterizedPlots] % len(color)])

                if plot_error_bar:
                    ax.fill_between(x_filtered, groups_my[group] - groups_sigmay, groups_my[group] + groups_sigmay, alpha = 0.2, color = color[rasterizedPlotsIndicies[rasterizedPlots] % len(color)])

                if plot_best_and_worst:
                    ax.fill_between(x_filtered, groups_min_y[group], groups_max_y[group], alpha = 0.3, color = color[rasterizedPlotsIndicies[rasterizedPlots] % len(color)])
    ...
    # Provide title
    ax.set_title(analysisView.edtGraphicsTitle.text())
...
def onChangeAutoLimitsInAxis():
    visibility = not analysisView.cbxAutoDefineAxisLimits.isChecked()
    for w in [analysisView.lblXMinXMax, analysisView.lblYMinYMax,
              analysisView.edtXmin, analysisView.edtXmax, analysisView.edtYmin, analysisView.edtYmax,
              analysisView.lblAxisSlahX, analysisView.lblAxisSlahY]:
        w.setEnabled(visibility)

def onHideShowSettingsInAnalysis():
    pass
...
def onTabWidgetInAnalysisChanged():
    ...
    analysisView.customPlotsStates[prev_index] = {"xaxe" : analysisView.cbxXaxis.currentIndex(), 
                                                  ...
                                                  "cbxDrawErrorBars": analysisView.cbxDrawErrorBars.checkState(),
                                                  "cbxDrawBestAndWorst": analysisView.cbxDrawBestAndWorst.checkState(),
                                                  ...
                                                  "spbColorShift": analysisView.spbColorShift.value(),
                                                  ...
                                                  "edtGraphicsTitle": analysisView.edtGraphicsTitle.text(),

                                                  "cbxLegendFormat" : analysisView.cbxLegendFormat.currentIndex(),
                                                  "cbxLineStyle" : analysisView.cbxLineStyle.currentIndex(),

                                                  "cbxAutoDefineAxisLimits": analysisView.cbxAutoDefineAxisLimits.checkState(),
                                                  "cbxLegendLogScalingY": analysisView.cbxLegendLogScalingY.checkState(),

                                                  "edtXmin" : analysisView.edtXmin.text(),
                                                  "edtXmax": analysisView.edtXmax.text(),
                                                  "edtYmin": analysisView.edtYmin.text(),
                                                  "edtYmax": analysisView.edtYmax.text()
                                                  }
    ...
    if index in analysisView.customPlotsStates:
        ...
        analysisView.cbxDrawErrorBars.setCheckState(s["cbxDrawErrorBars"])
        analysisView.cbxDrawBestAndWorst.setCheckState(s["cbxDrawBestAndWorst"])
        ...
        analysisView.spbColorShift.setValue(s["spbColorShift"])
        ...
        analysisView.edtGraphicsTitle.setText(s["edtGraphicsTitle"]),
        analysisView.cbxLegendFormat.setCurrentIndex(s["cbxLegendFormat"]),
        analysisView.cbxLineStyle.setCurrentIndex(s["cbxLineStyle"]),
        analysisView.cbxAutoDefineAxisLimits.setCheckState(s["cbxAutoDefineAxisLimits"]),
        analysisView.cbxLegendLogScalingY.setCheckState(s["cbxLegendLogScalingY"]),

        analysisView.edtXmin.setText(s["edtXmin"])
        analysisView.edtXmax.setText(s["edtXmax"])
        analysisView.edtYmin.setText(s["edtYmin"])
        analysisView.edtYmax.setText(s["edtYmax"])
...
analysisViewItems = vars(analysisView)
analysisViewItemIndex = 1
layouts_to_append_plots = []
while True:
    plotName = f"loCustomPlot_{analysisViewItemIndex}"
    if plotName in analysisViewItems:
        layouts_to_append_plots.append(getattr(analysisView, plotName))
    else:
        break
    analysisViewItemIndex += 1
analysisViewItems = None
...
analysisView.cbxAutoDefineAxisLimits.stateChanged.connect(onChangeAutoLimitsInAxis)
analysisView.btnHideShowRightPanel.clicked.connect(onHideShowSettingsInAnalysis)
...
mainView.actionLoadAdd.triggered.connect(onAddExperimentResults)
...
analysisView.btnRemoveAllExperimentsAnalsisWidget.clicked.connect(onRemoveAllExperiments)
...
analysisView.btnExecuteScript.setEnabled(False)
analysisView.btnExecuteScript.clicked.connect(onExecuteScript)
...
# Run main event loop
show_maximized = True
mainView.setWindowTitle("Simulator")
