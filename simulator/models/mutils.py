...
def print_models_info(model:torch.nn.Module, args):
    ...
    logger.info('{0:38s} | {1:3s} | {2:s} | {3:s}'.format("Name", "Layers", "Learnable Parameters(Frozen)", "Learnable Parameters(Train)"))
    logger.info("-------------------------------------------------------------------------------------------------")

    layers_info = torch_layers_info(model)
    for layer, info in layers_info.items():
        logger.info(f'{layer:38s} | {info["count"]:6d} | {( (info["parameters_size_bytes_cpu_frozen"] + info["parameters_size_bytes_gpu_frozen"])/1024.0):8g} KBytes / {(info["parameters_size_numel_cpu_frozen"] + info["parameters_size_numel_gpu_frozen"])} elements'
                    +
...
def set_params_to_zero(model, param_predicate = None):
    """
...
def set_params_uniform_random(model, a = 0.0, b = 1.0, param_predicate = None):
    """
...
def get_params(model:torch.nn.Module, param_predicate = None):
    """
...
def set_params(model:torch.nn.Module, parameters, param_predicate = None):
    """
...
def get_zero_gradient_compatible_with_model(model:torch.nn.Module):
    """
...
def add_to_gradient(model:torch.nn.Module, extra_grad):
    """
...
def set_gradient(model:torch.nn.Module, grad):
    """
    Set model gradient
...
def l2_norm_of_gradient_m(model:torch.nn.Module):
    value = 0.0
    for p in model.parameters():
        if p.grad is not None:
