...
# Import wandb support python api
import wandb
...
def main(args, raw_cmdline, extra_):
    ...
    if not args.evaluate:  # Training mode
        ...
        # Initialize local training threads in case of using multithread implementation
        exec_ctx.local_training_threads.adjust_num_workers(w=args.threadpool_for_local_opt, own_cuda_stream=True, device_list=args.gpu)
        ...
        exec_ctx.remote_training_threads.adjust_num_workers(w = len(external_devices), own_cuda_stream = False, device_list = ["cpu"])
        for th_i in range(len(exec_ctx.remote_training_threads.threads)):
            ...
            try:
                ...
                th.external_socket.rawSend(pickle.dumps(raw_cmdline))                    # provide original command line (it will be changed by the client) to be consistent with used devices
        ...
        # Initialize local training threads in case of using multithread implementation (0 is acceptable)
        exec_ctx.eval_thread_pool.adjust_num_workers(w = args.eval_async_threads, own_cuda_stream = True, device_list = args.gpu)
        ...
        model, criterion, round = get_training_elements(args.model_name, args.dataset, args, args.resume_from, args.load_best, args.device, args.loss)
...
def init_and_help_with_compute(args, raw_cmdline, trainloader, testloader, exec_ctx):
    ...
    is_rnn = args.model in RNN_MODELS
    # TODO: figure out how to exploit DataParallel. Currently - parallelism across workers
    model, criterion, current_round = get_training_elements(args.model, args.dataset, trainloader.dataset, args, args.resume_from, args.load_best, args.device, args.loss)
    #==================================================================================================================
    ...
    algorithms.evaluateGradient(None, model, trainloader, criterion, is_rnn, update_statistics=False, evaluate_function=False, device=args.device, args=args)
    gradient_at_start = mutils.get_gradient(model)
    for p in model.parameters():
        p.grad = None
    H = algorithms.initializeServerState(args, model, trainloader.dataset.num_clients, gradient_at_start)

    # Append all launch arguments (setuped and default)
    H["args"] = args
    ...
    # ==================================================================================================================
    local_optimiser = get_optimiser(model.parameters(), args.local_optimiser, args.local_lr, args.local_momentum, args.local_weight_decay)
    socket = exec_ctx.extra_
    state_dicts_thread_safe = Buffer()

    while True:
        # Main loop woith obtaining commands from master
        ...
        if cmd == "non_local_training" :
            ...
            for k, v in client_state['H'].items():
                if torch.is_tensor(v):
                    client_state['H'][k] = v.to(device)

            model_dict_original = model_dict_original.to(device)
...
def init_and_train_model(args, raw_cmdline, trainloader, testloader, exec_ctx):
    ...
    for th in exec_ctx.local_training_threads.threads:
        ...
        #th.model_copy, _  = initialise_model(args.model, args.dataset, trainloader.dataset, args, args.resume_from, args.load_best)
        #th.model_copy.load_state_dict(copy.deepcopy(model.state_dict()))
        th.model_copy = th.model_copy.to(th.device)

        th.local_optimiser_copy = get_optimiser(th.model_copy.parameters(), args.local_optimiser, args.local_lr,  args.local_momentum, args.local_weight_decay)
    ...
    if "Li_data_samples" in dir(trainloader.dataset):
        H["Li_data_samples"] = trainloader.dataset.Li_data_samples
    ...
    sampled_clients = get_sampled_clients(H, trainloader.dataset.num_clients, args, exec_ctx)
    # H["sampled_clients"] = sampled_clients

    # Main Algorithm Optimization loop
    for i in range(args.rounds):
        # Check requirement to force stop simulation
        if execution_context.is_simulation_need_ealystop_fn is not None:
            if execution_context.is_simulation_need_ealystop_fn(H):
                break
        ...
        metrics_meter, fed_dataset_clients = run_one_communication_round(H, model, trainloader, criterion, local_optimiser,
                                                                         optimiser_dict_original, global_optimiser, args.device,
                                                                         current_round, args.run_local_steps, args.number_of_local_iters,
        ...
        if (i % args.eval_every == 0 or i == (args.rounds - 1)) and metric_to_optim != "none":
            if args.eval_async_threads > 0:
                defered_eval_and_save_checkpoint(model, criterion, args, current_round, is_rnn=is_rnn, metric_to_optim=metric_to_optim, exec_ctx=exec_ctx)
                ...
                metrics = evaluate_model(model, testloader, criterion, args.device, current_round, print_freq=10, is_rnn=is_rnn, metric_to_optim=metric_to_optim)
                ...
                model_filename = '{model}_{run_id}_checkpoint_{round:0>2d}.pth.tar'.format(model=args.model, run_id=args.run_id, round=current_round)

                is_best = avg_metric > best_metric
                save_checkpoint(model, model_filename, is_best=is_best, args=args, metrics=metrics, metric_to_optim=metric_to_optim)
        ...
        if args.log_gpu_usage:
            ...
            for dev in args.gpu:
                if gpu_utils.is_target_dev_gpu(dev):
                    ...
                    logger.info(f"GPU usage: Before round {i} we have used {memory_gpu_used/(1024.0**2):.2f} MB from device {dev}")
        ...
        if current_round % args.eval_every == 0 or i == (args.rounds - 1):
            ...
            if "full_objective_value_train" in extra_opts and "full_gradient_norm_train" not in extra_opts:
                ...
                for c in fed_dataset_all_clients:
                    fed_dataset.set_client(c)
                    fvalue = algorithms.evaluateFunction(None, model, trainloader, criterion, is_rnn, update_statistics=False, device=args.device, args=args)
                    fvalue_avg = (fvalue_avg * c + fvalue)/(c + 1)
                H['history'][current_round]["full_objective_value_train"] = fvalue_avg
            #===========================================================================================================
            if "full_gradient_norm_val" in extra_opts and "full_objective_value_val" in extra_opts:
                fvalue = algorithms.evaluateGradient(None, model, testloader, criterion, is_rnn, update_statistics = False, evaluate_function = True, device = args.device, args=args)
            ...
            if "full_gradient_norm_val" in extra_opts and "full_objective_value_val" not in extra_opts:
                algorithms.evaluateGradient(None, model, testloader, criterion, is_rnn, update_statistics = False, evaluate_function = False, device = args.device, args=args)
                H['history'][current_round]["full_gradient_norm_val"] = mutils.l2_norm_of_gradient_m(model)

            if "full_objective_value_val" in extra_opts and "full_gradient_norm_val" not in extra_opts:
                fvalue = algorithms.evaluateFunction(None, model, testloader, criterion, is_rnn, update_statistics = False, device = args.device, args=args)
                H['history'][current_round]["full_objective_value_val"] = fvalue
            #===========================================================================================================
            # Interpolate
            #===========================================================================================================
            if current_round > 0:
                ...
                if i % args.eval_every == 0:
                    ...
                    pass                                # Exactly i%args.eval_every round beofre previous eval had place to be
                else:
                ...
                for s in range(1,look_behind):
                    ...
                    # Lineary interpolate between a (alpha=0.0) and b(alpha=1.0)
                    lerp = lambda a, b, alpha: (1.0 - alpha)*a + alpha * (b)

                    if "full_gradient_norm_train" in extra_opts:
                        H['history'][inter_round]["full_gradient_norm_train"] = lerp(H['history'][prev_eval_round]["full_gradient_norm_train"], H['history'][current_round]["full_gradient_norm_train"], alpha)
                    if "full_objective_value_train" in extra_opts:
                        H['history'][inter_round]["full_objective_value_train"] = lerp(H['history'][prev_eval_round]["full_objective_value_train"], H['history'][current_round]["full_objective_value_train"], alpha)
                    if "full_gradient_norm_val" in extra_opts:
                        H['history'][inter_round]["full_gradient_norm_val"] = lerp(H['history'][prev_eval_round]["full_gradient_norm_val"], H['history'][current_round]["full_gradient_norm_val"], alpha)
                    if "full_objective_value_val" in extra_opts:
                        H['history'][inter_round]["full_objective_value_val"] = lerp(H['history'][prev_eval_round]["full_objective_value_val"], H['history'][current_round]["full_objective_value_val"], alpha)
    ...
    # Not neeed for use, but for purpose of excluding any bugs inside PyTorch
    logger.info("Syncronize all GPU streams")
    if is_target_gpu:
    ...
    tensor_prune_mb_threshold = 25.0
...
def runSimulation(cmdline, extra_ = None):
    ...
    if args.deterministic:
        ...
        # Project use execution context random generatators for random and numpy in thread safe way
        if CUDA_SUPPORT:
...
class ClientThread(threading.Thread):
    ...
    def run(self):
        ...
        if cmd == "list_of_gpus":                              # List of resources in the system
            ...
            # Remove unnessesary flags for worker
            i = len(cmdLine) - 1
            while i >= 0:
                if cmdLine[i] == "--metric":
                    cmdLine[i+1] = "none"
                ...
                if cmdLine[i] in ['--wandb-key', '--eval-async-threads', '--save-async-threads', '--threadpool-for-local-opt', '--external-devices', '--evaluate']:
